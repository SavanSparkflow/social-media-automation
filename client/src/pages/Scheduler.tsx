...
const Scheduler = () => {
  ...
  const togglePlatform = (id: string) => setSelectedPlatform((prev) => (prev.includes(id) ? prev.filter((p) => p !== id) : [...prev, id]))
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <form className="space-y-5" onSubmit={handleSchedule}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {PLATFORMS.map((p) => {
                  ...
                  return (
                    <button key={p.id} type="button" onClick={() => togglePlatform(p.id)} className={`flex items-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active ? "bg-red-50 border-red-300 text-red-500 scale-103" : "border-slate-300 text-slate-500 hover:border-slate-300"}`}>
            ...
            <div>
              ...
              <textarea required rows={5} placeholder="what do you want to share today?" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm placeholder-slate-400 outline-none resize-none" value={content} onChange={(e) => setContent(e.target.value)} maxLength={280} />
